...
import com.kutedev.easemusicplayer.utils.ByteQueue
...
import uniffi.ease_client_backend.AssetChunk
    ...
    fun poll() {
        ...
        when (result) {
            is AssetChunkRead.Chunk -> {
                when (result.v1) {
                    is AssetChunk.Buffer -> {
                        _byteQueue.put(result.v1.v1)
                    }
                    is AssetChunk.Status -> {
        ...
        player.addListener(object : Player.Listener {
            override fun onIsPlayingChanged(isPlaying: Boolean) {
                if (isPlaying) {
                    ...
                    BackendBridge.sendPlayerEvent(PlayerDelegateEvent.Pause)
                }
            }
            ...
            override fun onPlaybackStateChanged(playbackState: Int) {
                if (playbackState == Player.STATE_ENDED) {
                    ...
                    BackendBridge.sendPlayerEvent(PlayerDelegateEvent.Loaded)
                    syncMetadataImpl()
                } else if (playbackState == Player.STATE_BUFFERING) {
                    BackendBridge.sendPlayerEvent(PlayerDelegateEvent.Loading)
                }
            }
