...
use crate::{
    ...
    view_models::playlist::page::PlaylistPageVM,
};
...
impl MusicControlVM {
    ...
    pub(crate) fn play(&self, cx: &ViewModelContext, id: MusicId) -> EaseResult<()> {
        let current_playlist_id = PlaylistPageVM::of(&cx).id();
    ...
    fn on_event(&self, cx: &ViewModelContext, event: &Action) -> EaseResult<()> {
        match event {
            ...
            Action::Widget(action) => match (&action.widget, &action.typ) {
                (Widget::MusicControlPause, WidgetActionType::Click) => {
                    self.pause(cx)?;
                }
                (Widget::MusicControlPlay, WidgetActionType::Click) => {
                    self.resume(cx)?;
                }
                (Widget::MusicControlStop, WidgetActionType::Click) => {
                    self.stop(cx)?;
                }
                (Widget::MusicControlPlayNext, WidgetActionType::Click) => {
                    self.play_next(cx)?;
                }
                (Widget::MusicControlPlayPrevious, WidgetActionType::Click) => {
                    self.play_previous(cx)?;
                }
                (Widget::MusicControlPlaymode, WidgetActionType::Click) => {
