...
use ease_client::{
    view_models::view_state::views::{
        playlist::{VCreatePlaylistState, VCurrentPlaylistState, VPlaylistListState},
...
pub struct ViewStates {
    ...
    pub playlist: Entity<VCurrentPlaylistState>,
...
impl ViewStates {
    pub fn new(cx: &mut gpui::App) -> Self {
        Self {
            ...
            playlist: cx.new(|_| Default::default()),
...
impl GpuiViewStateService {
    ...
    fn flush_impl(&self, cx: &mut gpui::App, v: ease_client::RootViewModelState) {
        self.flush_vs(cx, &v.current_playlist, &self.gpui_vs.playlist);
