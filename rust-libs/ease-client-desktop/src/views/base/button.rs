...
pub enum EaseButtonType {
    Primary,
    Default,
}

pub struct EaseButtonComponent<F>
...
impl<F> EaseButtonComponent<F>
where F: Fn(&mut gpui::WindowContext<'_>) {
    pub fn new(id: SharedString) -> Self {
        Self {
            id,
            typ: EaseButtonType::Default,
            on_click: None,
            text: Default::default(),
        }
    }
    fn typ(mut self, typ: EaseButtonType) -> Self {
        self.typ = typ;
        self
    }
...
impl<F> Render for EaseButtonComponent<F>
where
F: Fn(&mut gpui::WindowContext<'_>) + 'static {
    fn render(&mut self, cx: &mut ViewContext<Self>) -> impl IntoElement {
        let bg_col = match self.typ {
            EaseButtonType::Default => rgb(RGB_SLIGHT_300),
            EaseButtonType::Primary => rgb(RGB_PRIMARY),
        };
        let bg_hovered_col = match self.typ {
            EaseButtonType::Default => rgb(RGB_SLIGHT_700),
            EaseButtonType::Primary => rgb(RGB_PRIMARY_700),
        };
