...
fn plugin_id(args: &[JsValue]) -> JsResult<PluginId> {
    ...
    js_ctx.data::<PluginId>().ok_or_else(|| {
        JsError::from(
            JsNativeError::typ()
                .with_message("ease:secret: no plugin context bound to this instance"),
        )
    })
}
...
fn require_string(args: &[JsValue], idx: usize) -> JsResult<String> {
    ...
    let s = v.as_string().ok_or_else(|| {
        JsError::from(
            JsNativeError::typ()
                .with_message(format!("ease:secret: expected string at index {idx}")),
        )
    })?;
...
fn require_i64(args: &[JsValue], idx: usize) -> JsResult<i64> {
    ...
    let n = v.as_number().ok_or_else(|| {
        JsError::from(
            JsNativeError::typ()
                .with_message(format!("ease:secret: expected number at index {idx}")),
        )
    })?;
    Ok(n as i64)
}
...
fn unwrap_blocking<R>(description: &str, result: BResult<R>) -> JsResult<R> {
    result.map_err(|e| {
        JsError::from(
            JsNativeError::typ().with_message(format!("ease:secret {description} failed: {e:?}")),
        )
    })
}
