...
pub(crate) async fn cr_get_playlist(cx: Context, arg: PlaylistId) -> BResult<Option<Playlist>> {
    let conn = get_conn(&cx)?;
    let models = db_load_playlists(conn.get_ref())?;
    let first_covers = db_load_first_music_covers(conn.get_ref())?;
...
pub(crate) async fn ccu_upsert_playlist(cx: Context, arg: ArgUpdatePlaylist) -> BResult<()> {
    let conn = get_conn(&cx)?;
...
pub(crate) async fn cd_remove_playlist(cx: Context, arg: PlaylistId) -> BResult<()> {
    let conn = get_conn(&cx)?;
