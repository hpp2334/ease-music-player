...
- **First run** (`plugin.bootstrap`): Rust installs the bundled `com.ease.webdav` zip read **natively** from `assets/plugin-bundles/` via the NDK `AAssetManager` (the raw pointer is stashed by the `bindPluginRuntime` JNI trampoline — zip bytes never cross JNI), then any plugin referenced by existing storage rows (bundled, else best-effort from the default registry source). Uninstall wipes **all** plugin data — its storage rows (removed via the normal storage-removal cascade, taking their musics/playlist entries/cover blobs with them), its `plugin_kv` store and its scoped `plugin:<id>` secrets — so the "storage removed" rendering only covers legacy/orphaned rows from older versions.
